...
class PromotionService:
    ...
    def _initialize_sample_promotions(self):
        ...
        with SessionLocal() as db:
            promotion_repository.add_or_update(db, sample_promo)
    ...
    def calculate_promotions(self, user_id: str, cart_items: List[str], total_value: float) -> PromotionResponse:
        ...
        with SessionLocal() as db:
            active_promos = promotion_repository.list_active_promotions(db)

            for promo_data in active_promos:
                if promo_data.product_id in cart_items and self._check_conditions(promo_data.conditions, total_value):
                    promotion = Promotion(
                        product_id=promo_data.product_id,
                        discount_percentage=promo_data.discount_percentage,
                        discount_type=promo_data.discount_type,
                        discount_value=promo_data.discount_value,
                        upsell_product_ids=promo_data.upsell_product_ids or [],
                        conditions=promo_data.conditions or {}
                    )
